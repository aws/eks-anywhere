...
# Before using the script, please ensure that EKSA_VSPHERE_PASSWORD env var has been updated.
set -o errexit
set -o nounset
set -o pipefail
...
encoded_cloud_provider_vsphere_credential="$(cat tmp-secret.yaml | base64)"
...
if [[ $(kubectl get secrets -n eksa-system vsphere-credentials -o jsonpath='{.data.passwordCSI}') ]]; then
  kubectl patch -n eksa-system secrets "$cluster_name-cloud-provider-vsphere-credentials" --patch="{\"data\":{\"passwordCSI\":\"$encoded_password\"}}"
fi
...
# Patch {CLUSTER_NAME}-csi-vsphere-config if it exists
if $(kubectl get secrets -n eksa-system "$cluster_name-csi-vsphere-config"); then
  kubectl patch -n eksa-system secrets "$cluster_name-csi-vsphere-config" --patch="{\"data\":{\"password\":\"$encoded_password\"}}"
  last_applied=$(kubectl get secrets -n eksa-system "$cluster_name-csi-vsphere-config" -o jsonpath='{.metadata.annotations.kubectl\.kubernetes\.io/last-applied-configuration}')
  if [[ $last_applied ]]; then
    new_annotation=$(echo $last_applied | jq -c --arg password $encoded_password '.data.password=$password')
    kubectl annotate --overwrite -n eksa-system secrets "$cluster_name-csi-vsphere-config" kubectl.kubernetes.io/last-applied-configuration=$new_annotation
  fi
else
  echo "$cluster_name-csi-vsphere-config does not exists"
fi

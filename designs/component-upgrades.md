...
* We don’t introduce more concepts/commands. Customers just have to use the same command they are already used to. The syntax stays the same and there is no need to define what needs to be upgraded and to what version, the cli takes care of this transparently.
...
The cli needs the ability to detect both a component version changes as well as new builds of the same versioned component (eg. new container build due to a patch in a dependency like AL2). The process to upgrade some components could be different for these two scenarios.

### Order of execution

Components should be **upgraded** **before the new Kubernetes version is rolled out** (if needed). It’s more common for an old component to have problems with a new Kubernetes version than vice-versa. There might be case by case exceptions, right now there is not, at least with the current set of core components.
...
We have to be careful and make sure that any changes to the CRDs are backwards compatible, since at this point there are already eks-a objects in the cluster. Examples of not backwards compatible changes: adding a required field, changing the type of field, making an existing field required, etc. It’s important to note that applying the manifest with the new CRD won’t fail, even if the changes are not backwards compatible. But the objects already in the cluster will become invalid, so any kind of update to those (eg. flux reconciliation) will be be rejected by the kube api server.
...
If there is a new eks-a api version, we will need to write conversion webhooks.
...
1. Generate pre-flight check with helm and apply it with kubectl. This “pre-pulls" the new images on the nodes to avoid `ErrImagePull` errors later during the upgrade
...
As mentioned in the [CAPI](#capi) section, we don’t don’t need any extra process to perform CAPI component upgrades that involve an api version change. However, there will some extra effort needed to support `v1alpha4` in the cli. Most of this will probably apply to future api version changes.
...
There are not backwards breaking changes. However, the expected behavior of the `upgrade cluster` command will be changing, so it should be clearly communicated in the the changelog.
...
* Create the cluster using the previous minor release and upgrade them using the latest version from the current branch
    * For `main`: this makes sure that the next (future) minor release it’s compatible with clusters created by the present latest release
    * When running this in a release branch: this makes sure that the next (future) patch release it’s compatible with clusters created by the previous minor release

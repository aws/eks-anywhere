...
.PHONY: capc-test
capc-test: e2e ## Run default e2e capc test locally
	./bin/e2e.test -test.v -test.run $(CLOUDSTACK_E2E_TEST)
...
.PHONY: cloudstack-e2e-test
cloudstack-e2e-test: e2e ## Run cloudstack integration test in provisioned cloudstack env
	scripts/e2e_test_cloudstack.sh $(CLOUDSTACK_E2E_TEST)
...
.PHONY: capc-test-all
capc-test-all: capc-test capc-test-120

.PHONY: capc-test-%
capc-test-%: e2e ## Run CAPC tests
	./bin/e2e.test -test.v -test.run TestCloudStackKubernetes$*SimpleFlow
...
mocks: ## Generate mocks
	...
	${GOPATH}/bin/mockgen -destination=pkg/providers/tinkerbell/hardware/mocks/json.go -package=mocks -source "pkg/providers/tinkerbell/hardware/json.go" TinkerbellHardwareJsonFactory
...
.PHONY: conformance-cloudstack
conformance-cloudstack:
	$(MAKE) e2e-tests-binary E2E_TAGS=conformance_e2e
	./bin/e2e.test -test.v -test.run 'TestCloudStackKubernetes121ThreeWorkersConformanc.*'

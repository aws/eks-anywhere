...
func (r *Reconciler) Reconcile(ctx context.Context, logger logr.Logger, client client.Client, spec *cluster.Spec) (res controller.Result, reterr error) {
	...
	if !ciliumCfg.IsManaged() {
		...
		conditions.MarkFalse(spec.Cluster, anywherev1.DefaultCNIConfiguredCondition, anywherev1.SkipUpgradesForDefaultCNIConfiguredReason, anywherev1.ConditionSeverityWarning, "Configured to skip default Cilium CNI upgrades")
		return controller.Result{}, nil
	}
	...
	if upgradeInfo.VersionUpgradeNeeded() {
		...
		if result, err := r.upgrade(ctx, logger, client, installation, spec); err != nil {
			...
			conditions.MarkFalse(spec.Cluster, anywherev1.DefaultCNIConfiguredCondition, anywherev1.DefaultCNIUpgradeInProgressReason, anywherev1.ConditionSeverityInfo, "Cilium version upgrade needed")
			return result, nil
		}

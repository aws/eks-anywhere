...
// getMachineCertificateInfo is a helper function to check certificate expiry for a set of machines.
func (s *Scanner) getMachinesCertificateInfo(machines []MachineInfo, port string) []anywherev1.ClusterCertificateInfo {
	...
	for _, machine := range machines {
		...
		if err != nil {
			s.logger.Error(err, "checking certificate expiry", "component", machine.Name, "ip", machine.IP, "port", port)
			continue
		}
...
func (s *Scanner) CheckCertificateExpiry(ctx context.Context, cluster *anywherev1.Cluster) ([]anywherev1.ClusterCertificateInfo, error) {
	...
	if cluster.Spec.ExternalEtcdConfiguration != nil {
		...
		if err != nil {
			s.logger.Error(err, "getting etcd machines")
			return certificateInfos, nil // Don't fail the entire operation
		}
	...
	if err != nil {
		s.logger.Error(err, "getting control plane machines")
		return nil, err
	}
...
// getControlPlaneMachines retrieves control plane machine names and IPs.
func (s *Scanner) getControlPlaneMachines(ctx context.Context, cluster *anywherev1.Cluster) ([]MachineInfo, error) {
	var machines []MachineInfo

	// Get machines with control-plane selector
	...
	if len(machines) == 0 {
		// Fallback to using the control plane endpoint from cluster spec
		if cluster.Spec.ControlPlaneConfiguration.Endpoint != nil {
			machines = append(machines, MachineInfo{
...
// getEtcdMachines retrieves external etcd machine names and IPs.
func (s *Scanner) getEtcdMachines(ctx context.Context, cluster *anywherev1.Cluster) ([]MachineInfo, error) {
	var machines []MachineInfo

	// Get machines with etcd selector
	machineList := &clusterv1.MachineList{}
	selector := client.MatchingLabels{
		"cluster.x-k8s.io/cluster-name": cluster.Name,
		"cluster.x-k8s.io/etcd-cluster": cluster.Name + "-etcd",
	}

...
func TranslateAll(reader MachineReader, writer MachineWriter) error {
	for {
		err := Translate(reader, writer)
		if err == io.EOF {
			return nil
		}
...
func Translate(reader MachineReader, writer MachineWriter) error {

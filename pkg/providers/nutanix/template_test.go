...
func TestNewNutanixTemplateBuilderExternalEtcdWithOptional(t *testing.T) {
	...
	expectedControlPlaneSpec, err := os.ReadFile("testdata/expected_results_external_etcd_with_optional.yaml")
	require.NoError(t, err)
	assert.Equal(t, expectedControlPlaneSpec, cpSpec)
	...
	expectedWorkersSpec, err := os.ReadFile("testdata/expected_results_external_etcd_with_optional_md.yaml")
	require.NoError(t, err)
	assert.Equal(t, expectedWorkersSpec, workerSpec)
}

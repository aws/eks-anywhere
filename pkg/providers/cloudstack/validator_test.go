...
func TestValidateCloudStackDatacenterConfig(t *testing.T) {
	...
	cmk.EXPECT().ValidateZonePresent(ctx, !cloudstackDatacenter.Spec.Insecure, cloudstackDatacenter.Spec.Zone).Return(nil)
...
func TestValidateCloudStackConnection(t *testing.T) {
	...
	cmk.EXPECT().ValidateCloudStackConnection(ctx, true).Return(nil)
	err := validator.validateCloudStackAccess(ctx, true)
	if err != nil {
		t.Fatalf("failed to validate CloudStackDataCenterConfig: %v", err)
	}
}
...
func TestSetupAndValidateUsersNil(t *testing.T) {
	...
	cmk.EXPECT().ValidateServiceOfferingPresent(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(3)
	cmk.EXPECT().ValidateAffinityGroupsPresent(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Times(3)
...
func TestValidateMachineConfigsHappyCase(t *testing.T) {
	...
	validator := NewValidator(cmk)
	cmk.EXPECT().ValidateTemplatePresent(ctx, true, datacenterConfig.Spec.Domain,
		datacenterConfig.Spec.Zone, datacenterConfig.Spec.Account, testTemplate).Times(3)
	cmk.EXPECT().ValidateServiceOfferingPresent(ctx, true, datacenterConfig.Spec.Domain,
		datacenterConfig.Spec.Zone, datacenterConfig.Spec.Account, testOffering).Times(3)
	cmk.EXPECT().ValidateAffinityGroupsPresent(ctx, true, datacenterConfig.Spec.Domain,
...
func TestValidateCloudStackMachineConfig(t *testing.T) {
	...
	for _, machineConfig := range machineConfigs {
		cmk.EXPECT().ValidateTemplatePresent(ctx, true, "domain1", testZone, "admin", machineConfig.Spec.Template).Return(nil)
		cmk.EXPECT().ValidateServiceOfferingPresent(ctx, true, "domain1", testZone, "admin", machineConfig.Spec.ComputeOffering).Return(nil)
		if len(machineConfig.Spec.AffinityGroupIds) > 0 {
			cmk.EXPECT().ValidateAffinityGroupsPresent(ctx, true, "domain1", testZone, "admin", machineConfig.Spec.AffinityGroupIds).Return(nil)
		}

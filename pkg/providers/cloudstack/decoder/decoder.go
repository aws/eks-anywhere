...
func ParseCloudStackCredsFromSecrets(secrets []apiv1.Secret) (*CloudStackExecConfig, error) {
	...
	for _, secret := range secrets {
		...
		if !ok {
			return nil, fmt.Errorf("secret %s is missing required key %s", secret.Name, VerifySslKey)
		}
	...
	if len(cloudstackProfiles) == 0 {
		return nil, fmt.Errorf("no secrets provided - unable to generate CloudStackProfiles")
	}

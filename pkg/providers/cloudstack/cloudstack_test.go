...
func TestNeedNewMachineTemplateAffinity(t *testing.T) {
	clusterSpec := givenClusterSpec(t, testClusterConfigMainFilename)
	cc := givenClusterConfig(t, testClusterConfigMainFilename)
	fillClusterSpecWithClusterConfig(clusterSpec, cc)
	dcConfig := givenDatacenterConfig(t, testClusterConfigMainFilename)
	...
	newMachineConfigsMap["test"].Spec.Affinity = "test-val"
	assert.True(t, NeedNewMachineTemplate(dcConfig, newDcConfig, machineConfigsMap["test"], newMachineConfigsMap["test"], test.NewNullLogger()), "Should have an immutable field changed")
}

...
func (p *vsphereProvider) waitForIPAMCRDs(ctx context.Context, cluster *types.Cluster) error {
	...
	kubectl := executables.NewExecutable("kubectl")

	for _, crdName := range crdsToWait {
		err := p.Retrier.Retry(func() error {
			// Check if the CRD exists by trying to get it
			_, err := kubectl.Execute(ctx, "get", "crd", crdName, "--kubeconfig", cluster.KubeconfigFile)
			return err
		})
		if err != nil {

...
func (p *vsphereProvider) setupSSHAuthKeysForCreate() error {
	...
	if len(p.controlPlaneSshAuthKey) <= 0 {
		logger.Info("Provided control plane machineconfig sshAuthorizedKey is not set or is empty, auto-generating new key pair...")
	...
	for _, workerNodeGroupConfiguration := range p.clusterConfig.Spec.WorkerNodeGroupConfigurations {
		...
		logger.Info("Provided worker machineconfig sshAuthorizedKey is not set or is empty, auto-generating new key pair...")
		...
		if len(p.workerSshAuthKey) <= 0 {
			if useKeyGeneratedForControlplane { // use the same key
				...
				logger.Info("Provided etcd machineconfig sshAuthorizedKey is not set or is empty, auto-generating new key pair...")
				generatedKey, err := common.GenerateSSHAuthKey(workerUser.Name, p.writer)
				if err != nil {
					return err
				}
	...
	if p.clusterConfig.Spec.ExternalEtcdConfiguration != nil {
		...
		if len(p.etcdSshAuthKey) <= 0 {
			if useKeyGeneratedForControlplane { // use the same key as for controlplane
				...
				generatedKey, err := common.GenerateSSHAuthKey(etcdUser.Name, p.writer)
				if err != nil {
					return err
				}

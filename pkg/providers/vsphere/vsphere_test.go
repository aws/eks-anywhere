...
func TestSetupAndValidateCreateClusterOsFamilyInvalid(t *testing.T) {
	...
	provider := givenProvider(t)
	controlPlaneMachineConfigName := clusterSpec.Cluster.Spec.ControlPlaneConfiguration.MachineGroupRef.Name
	provider.machineConfigs[controlPlaneMachineConfigName].Spec.OSFamily = "fedora"
	var tctx testContext
	tctx.SaveContext()
	err := provider.SetupAndValidateCreateCluster(ctx, clusterSpec)
	thenErrorExpected(t, "control plane osFamily: fedora is not supported, please use one of the following: bottlerocket, ubuntu, redhat", err)
}
...
func TestSetupAndValidateCreateClusterOsFamilyInvalidWorkerNode(t *testing.T) {
	...
	provider := givenProvider(t)
	workerMachineConfigName := clusterSpec.Cluster.Spec.WorkerNodeGroupConfigurations[0].MachineGroupRef.Name
	provider.machineConfigs[workerMachineConfigName].Spec.OSFamily = "fedora"
	var tctx testContext
	tctx.SaveContext()
	err := provider.SetupAndValidateCreateCluster(ctx, clusterSpec)
	thenErrorExpected(t, "worker node osFamily: fedora is not supported, please use one of the following: bottlerocket, ubuntu, redhat", err)
}
...
func TestSetupAndValidateCreateClusterOsFamilyInvalidEtcdNode(t *testing.T) {
	...
	provider := givenProvider(t)
	etcdMachineConfigName := clusterSpec.Cluster.Spec.ExternalEtcdConfiguration.MachineGroupRef.Name
	provider.machineConfigs[etcdMachineConfigName].Spec.OSFamily = "fedora"
	var tctx testContext
	tctx.SaveContext()
	err := provider.SetupAndValidateCreateCluster(ctx, clusterSpec)
	thenErrorExpected(t, "etcd node osFamily: fedora is not supported, please use one of the following: bottlerocket, ubuntu, redhat", err)
}

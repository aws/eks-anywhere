...
func TestTinkerbellClusterDefaultCastFail(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	config := &TinkerbellCluster{}

	// Call Default with the wrong type
	err := config.Default(context.TODO(), wrongType)

	g.Expect(err).To(gomega.HaveOccurred())
	g.Expect(err.Error()).To(gomega.ContainSubstring("expected a TinkerbellCluster"))
}

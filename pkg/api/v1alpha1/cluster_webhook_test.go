...
func TestClusterValidateCreateWorkloadPauseAnnotation(t *testing.T) {
	features.ClearCache()
	cOld := createCluster()
	g := NewWithT(t)
	cOld.SetManagedBy("management-cluster")
	cOld.Annotations = map[string]string{}
	cOld.Annotations[pausedAnnotation] = "true"
	g.Expect(cOld.ValidateCreate()).To(Succeed())
}

func TestClusterValidateCreateMgmtPauseAnnotation(t *testing.T) {
	features.ClearCache()
	cOld := createCluster()
	g := NewWithT(t)
...
func TestClusterValidateCreateActiveFeatureWorkload(t *testing.T) {
	features.ClearCache()
	cOld := createCluster()
	g := NewWithT(t)
	cOld.SetManagedBy("management-cluster")
...
func TestClusterValidateCreateActiveFeatureMgmt(t *testing.T) {
	features.ClearCache()
	cOld := createCluster()
	g := NewWithT(t)
	cOld.SetManagedBy("mgmt")
...
func TestClusterValidateCreatePassWorkloadCluster(t *testing.T) {
	features.ClearCache()
	t.Setenv(features.FullLifecycleAPIEnvVar, "true")
	cOld := createCluster()
	cOld.SetManagedBy("management-cluster")
...
func TestClusterValidateCreateFail(t *testing.T) {
	...
	strings.Contains(cOld.ValidateCreate().Error(), "control plane node count must be positive")
}

...
func TestVSphereMachineValidateUpdateOSFamilyImmutable(t *testing.T) {
	...
	g := NewWithT(t)
	_, err := c.ValidateUpdate(&vOld)
	g.Expect(err).To(MatchError(ContainSubstring("spec.osFamily: Forbidden: field is immutable")))
}
...
func TestVSphereMachineValidateUpdateInvalidType(t *testing.T) {
	...
	g := NewWithT(t)
	_, err := c.ValidateUpdate(vOld)
	g.Expect(err).To(MatchError(ContainSubstring("expected a VSphereMachineConfig but got a *v1alpha1.Cluster")))
}
...
func TestVSphereMachineValidateUpdateStoragePolicyImmutable(t *testing.T) {
	...
	g := NewWithT(t)
	_, err := c.ValidateUpdate(&vOld)
	g.Expect(err).To(MatchError(ContainSubstring("spec.storagePolicyName: Forbidden: field is immutable")))
}
...
func TestVSphereMachineConfigValidateCreateResourcePoolNotSet(t *testing.T) {
	...
	g := NewWithT(t)
	_, err := config.ValidateCreate()
	g.Expect(err).To(MatchError(ContainSubstring("resourcePool is not set or is empty")))
}
...
func TestVSphereMachineConfigValidateCreateTemplateNotSet(t *testing.T) {
	...
	g := NewWithT(t)
	_, err := config.ValidateCreate()
	g.Expect(err).To(MatchError(ContainSubstring("template field is required")))
}

...
func TestGetAndValidateClusterConfig(t *testing.T) {
	tests := []struct {
		...
		{
			testName:    "invalid node startup timeout",
			fileName:    "testdata/invalidMHCTimeout.yaml",
			wantCluster: nil,
			wantErr:     true,
		},
		{
			testName:    "invalid unhealthy machine timeout",
			fileName:    "testdata/invalidMHCMachineTimeout.yaml",
			wantCluster: nil,
			wantErr:     true,
		},
		{
			testName: "valid MHC timeouts",
			fileName: "testdata/validMHCTimeout.yaml",
			wantCluster: &Cluster{
				TypeMeta: metav1.TypeMeta{
					Kind:       ClusterKind,
					APIVersion: SchemeBuilder.GroupVersion.String(),

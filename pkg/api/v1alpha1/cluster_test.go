...
func TestValidateMDUpgradeRolloutStrategy(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "rolling upgrade knobs not specified",
			wantErr: "WorkerNodeGroupConfiguration: maxSurge and maxAvailable not specified or are 0. maxSurge and maxUnavailable cannot both be 0",
			cluster: &Cluster{
				Spec: ClusterSpec{
					WorkerNodeGroupConfigurations: []WorkerNodeGroupConfiguration{{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			err := validateMDUpgradeRolloutStrategy(tt.cluster)
			if tt.wantErr == "" {
				g.Expect(err).To(BeNil())
			} else {
				g.Expect(err).To(MatchError(ContainSubstring(tt.wantErr)))

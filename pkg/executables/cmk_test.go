...
func TestCmkListOperations(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cmkFunc: func(cmk executables.Cmk, ctx context.Context) error {
				err := cmk.ValidateZonePresent(ctx, zones[0])
				return err
			},
			cmkResponseError:      nil,
			wantErr:               false,
		...
		},
		{
			...
			cmkFunc: func(cmk executables.Cmk, ctx context.Context) error {
				err := cmk.ValidateZonePresent(ctx, zones[2])
				return err
			},
			cmkResponseError:      nil,
			wantErr:               false,
		...
		},
		{
			...
			cmkFunc: func(cmk executables.Cmk, ctx context.Context) error {
				return cmk.ValidateNetworkPresent(ctx, domainId, zones[2], accountName)
			},
			cmkResponseError:      nil,
			wantErr:               false,
			shouldSecondCallOccur: false,
		...
		},
		{
			...
			cmkFunc: func(cmk executables.Cmk, ctx context.Context) error {
				return cmk.ValidateNetworkPresent(ctx, domainId, zones[3], accountName)
			},
			cmkResponseError:      nil,
			wantErr:               false,
			shouldSecondCallOccur: true,

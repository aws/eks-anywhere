...
func TestKubectlGetEKSAClusters(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.testName, func(t *testing.T) {
			fileContent := test.ReadFile(t, tt.jsonResponseFile)
			k, ctx, cluster, e := newKubectl(t)
			e.EXPECT().Execute(ctx, []string{"get", "clusters.anywhere.eks.amazonaws.com", tt.clusterName, "-o", "json", "--kubeconfig", cluster.KubeconfigFile, "--namespace", clusterSpec.Namespace}).Return(*bytes.NewBufferString(fileContent), nil)

			gotCluster, err := k.GetEksaCluster(ctx, cluster, clusterSpec.Namespace)
			if err != nil {

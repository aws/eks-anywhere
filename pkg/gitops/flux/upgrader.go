...
func (f *Flux) Upgrade(ctx context.Context, managementCluster *types.Cluster, currentVersionsBundle, newVersionsBundle *releasev1.VersionsBundle, currentSpec *cluster.Spec, newSpec *cluster.Spec) (*types.ChangeDiff, error) {
	logger.V(1).Info("Checking for Flux upgrades")

	changeDiff := ChangeDiff(currentVersionsBundle, newVersionsBundle, currentSpec, newSpec)
	if changeDiff == nil {
	...
	if err := f.upgradeFilesAndCommit(ctx, newSpec); err != nil {
		return nil, fmt.Errorf("upgrading Flux from version %s to %s: %v", currentVersionsBundle.Flux.Version, newVersionsBundle.Flux.Version, err)
	}
...
func ChangeDiff(currentVersionsBundle, newVersionsBundle *releasev1.VersionsBundle, currentSpec, newSpec *cluster.Spec) *types.ChangeDiff {
	...
	oldVersion := currentVersionsBundle.Flux.Version

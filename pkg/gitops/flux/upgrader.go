...
func (f *Flux) Upgrade(ctx context.Context, managementCluster *types.Cluster, currentManagementComponentsVersionsBundle *releasev1.VersionsBundle, currentSpec *cluster.Spec, newSpec *cluster.Spec) (*types.ChangeDiff, error) {
	...
	if err := f.upgradeFilesAndCommit(ctx, newSpec); err != nil {
		return nil, fmt.Errorf("upgrading Flux from EKS-A version %s to EKS-A version %s: %v", currentSpec.Cluster.GetManagementComponentsVersion(), *newSpec.Cluster.Spec.EksaVersion, err)
	}

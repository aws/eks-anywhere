...
// GitClient is responsible for provider-agnostic actions on a generic git repository.
// Any actions that would be take with a git CLI executable should be included in the GitClient
...
// GitProviderClient is responsible for provider-dependant actions on a hosted git provider API
// Any actions you would take via the API of a git hosting provider should be included in the GitProviderClient
...
func (f *FluxAddonClient) InstallGitOps(ctx context.Context, cluster *types.Cluster, clusterSpec *cluster.Spec, datacenterConfig providers.DatacenterConfig, machineConfigs []providers.MachineConfig) error {
	...
	err := f.retrier.Retry(func() error {
		return f.gitClient.Pull(ctx, fc.branch())
	})
	if err != nil {
...
func (fc *fluxForCluster) syncGitRepo(ctx context.Context) error {
	...
	if !validations.FileExists(path.Join(f.writer.Dir(), ".git")) {
		r, err := fc.clone(ctx)
		if err != nil {
...
func (fc *fluxForCluster) initEksaWriter() (filewriter.FileWriter, error) {
	...
	w, err := fc.writer.WithDir(eksaSystemDir)
	if err != nil {
...
func (fc *fluxForCluster) initFluxWriter() (filewriter.FileWriter, error) {
	w, err := fc.writer.WithDir(fc.fluxSystemDir())
	if err != nil {
...
func (fc *fluxForCluster) clone(ctx context.Context) (*git.Repository, error) {
	...
	err = fc.FluxAddonClient.retrier.Retry(func() error {
		r, err = fc.gitProviderClient.GetRepo(ctx)
		return err
	})
	if err != nil {
	...
	if r != nil {
		...
		err = fc.gitClient.Branch(fc.branch())
		if err != nil {
			return nil, err
		}
		return r, nil
...
func (fc *fluxForCluster) createRemoteRepository(ctx context.Context) error {
	...
	err := fc.FluxAddonClient.retrier.Retry(func() error {
		_, err := fc.gitProviderClient.CreateRepo(ctx, opts)
		return err
	})
	if err != nil {
...
func (fc *fluxForCluster) validateRemoteConfigPathDoesNotExist(ctx context.Context) error {
	if fc.clusterSpec.Cluster.IsSelfManaged() {
		if exists, err := fc.gitProviderClient.PathExists(ctx, fc.owner(), fc.repository(), fc.branch(), fc.path()); err != nil {

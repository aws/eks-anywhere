...
func processCloudStackDatacenter(c *Config, objects ObjectLookup) error {
	if c.Cluster.Spec.DatacenterRef.Kind == anywherev1.CloudStackDatacenterKind {
		...
		if datacenter == nil {
			return fmt.Errorf("no %s named %s", anywherev1.CloudStackDatacenterKind, c.Cluster.Spec.DatacenterRef.Name)
		}

...
func (c *createTestSetup) expectInstallEksaComponentsBootstrap(err1, err2, err3 error) {
	gomock.InOrder(
		...
		c.eksdInstaller.EXPECT().InstallEksdManifest(
			c.ctx, c.clusterSpec, c.bootstrapCluster).Return(err3),
	)
}
...
func TestCreateRunSuccess(t *testing.T) {
	...
	test.expectInstallEksaComponentsBootstrap(nil, nil, nil)
...
func TestCreateInstallEksdManifestFailure(t *testing.T) {
	...
	err := errors.New("test")
	c.expectInstallEksaComponentsBootstrap(nil, nil, err)

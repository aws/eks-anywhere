...
// 			c.ctx, c.clusterSpec, c.bootstrapCluster, c.provider).Return(err1),

// 		c.eksdInstaller.EXPECT().InstallEksdCRDs(c.ctx, c.clusterSpec, c.bootstrapCluster).Return(err2),

// 		c.clusterManager.EXPECT().CreateEKSAReleaseBundle(
// 			c.ctx, c.bootstrapCluster, c.clusterSpec).Return(err3),
...
// 	err := errors.New("test")
// 	c.expectInstallEksaComponentsBootstrap(nil, nil, nil, err)

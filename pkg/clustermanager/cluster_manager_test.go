...
func TestClusterManagerUpgradeWorkloadClusterSuccess(t *testing.T) {
	...
	tt.mocks.provider.EXPECT().BuildMapForWorkerNodeGroupsByName(tt.clusterSpec.Spec.WorkerNodeGroupConfigurations)
	tt.mocks.provider.EXPECT().Name()
	tt.mocks.client.EXPECT().DeleteOldWorkerNodeGroup(tt.ctx, tt.cluster.KubeconfigFile, gomock.Any(), gomock.Any())

...
func TestClusterManagerCreateWorkloadClusterSuccessWithExtraObjects(t *testing.T) {
	...
	got, err := c.CreateWorkloadCluster(ctx, mgmtCluster, clusterSpec, m.provider)
	if err != nil {
		t.Errorf("ClusterManager.CreateWorkloadCluster() error = %v, wantErr nil", err)
	}
...
func TestClusterManagerUpgradeSelfManagedClusterSuccess(t *testing.T) {
	...
	tt.mocks.client.EXPECT().GetMachineDeployments(tt.ctx).Return([]clusterv1.MachineDeployment{}, nil)
...
func TestClusterManagerMoveCAPISuccess(t *testing.T) {
	...
	m.client.EXPECT().GetMachineDeployments(ctx)

...
func TestGetCuratedPackagesRegistries_RegistryMirror(t *testing.T) {
	tests := []struct {
		name                         string
		registryMirror               *registrymirror.RegistryMirror
		expectedSourceRegistry       string
		expectedDefaultRegistry      string
		expectedDefaultImageRegistry string
	}{
		{
			name: "registry mirror with regional private registry",
			registryMirror: &registrymirror.RegistryMirror{
				BaseRegistry: "1.2.3.4:443",
				NamespacedRegistryMap: map[string]string{
					constants.DefaultCoreEKSARegistry:              "1.2.3.4:443",
					"346438352937.dkr.ecr.us-west-2.amazonaws.com": "1.2.3.4:443/curated-packages",
				},
			},
			expectedSourceRegistry:       "1.2.3.4:443/eks-anywhere",
			expectedDefaultRegistry:      "1.2.3.4:443/curated-packages",
			expectedDefaultImageRegistry: "1.2.3.4:443/curated-packages",
		},
		{
			name: "registry mirror with non-regional default registry matching regex pattern",
			registryMirror: &registrymirror.RegistryMirror{
				BaseRegistry: "1.2.3.4:443",
				NamespacedRegistryMap: map[string]string{
					constants.DefaultCoreEKSARegistry:              "1.2.3.4:443",
					"783794618700.dkr.ecr.us-west-2.amazonaws.com": "1.2.3.4:443/curated-packages",
				},
			},
			expectedSourceRegistry:       "1.2.3.4:443/eks-anywhere",
			expectedDefaultRegistry:      "1.2.3.4:443/eks-anywhere",
			expectedDefaultImageRegistry: "1.2.3.4:443/curated-packages",
		},
		{
			name: "registry mirror with both regional and non-regional registries",
			registryMirror: &registrymirror.RegistryMirror{
				BaseRegistry: "1.2.3.4:443",
				NamespacedRegistryMap: map[string]string{
					constants.DefaultCoreEKSARegistry:              "1.2.3.4:443",
					"783794618700.dkr.ecr.us-west-2.amazonaws.com": "1.2.3.4:443/curated-packages-nonregional",
					"346438352937.dkr.ecr.us-west-2.amazonaws.com": "1.2.3.4:443/curated-packages-regional",
				},
			},
			expectedSourceRegistry:       "1.2.3.4:443/eks-anywhere",
			expectedDefaultRegistry:      "1.2.3.4:443/curated-packages-regional",
			expectedDefaultImageRegistry: "1.2.3.4:443/curated-packages-regional",
		},
		{
			name: "registry mirror with no matching curated packages registry",
			registryMirror: &registrymirror.RegistryMirror{
				BaseRegistry: "1.2.3.4:443",
				NamespacedRegistryMap: map[string]string{
					constants.DefaultCoreEKSARegistry: "1.2.3.4:443",
					"some.other.registry.com":         "1.2.3.4:443/other",
				},
			},
			expectedSourceRegistry:       "1.2.3.4:443/eks-anywhere",
			expectedDefaultRegistry:      "public.ecr.aws/eks-anywhere",
			expectedDefaultImageRegistry: "783794618700.dkr.ecr.us-west-2.amazonaws.com",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := curatedpackages.NewPackageControllerClient(
				cm, k, clusterName, kubeConfig, chart, tt.registryMirror,
				curatedpackages.WithManagementClusterName(clusterName),
				curatedpackages.WithValuesFileWriter(writer),
			)

			sourceRegistry, defaultRegistry, defaultImageRegistry := client.GetCuratedPackagesRegistries(context.Background())

			g.Expect(sourceRegistry).To(Equal(tt.expectedSourceRegistry))
			g.Expect(defaultRegistry).To(Equal(tt.expectedDefaultRegistry))
			g.Expect(defaultImageRegistry).To(Equal(tt.expectedDefaultImageRegistry))
		})
	}
}

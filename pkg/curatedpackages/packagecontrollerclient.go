...
// ChartInstallationDeleter handles deleting chart installations.
type ChartInstallationDeleter interface {
...
func NewPackageControllerClientFullLifecycle(logger logr.Logger, chartInstaller ChartInstaller, deleter ChartInstallationDeleter, kubectl KubectlRunner) *PackageControllerClient {
...
func NewPackageControllerClient(chartInstaller ChartInstaller, deleter ChartInstallationDeleter, kubectl KubectlRunner, clusterName, kubeConfig string, chart *releasev1.Image, registryMirror *registrymirror.RegistryMirror, options ...PackageControllerClientOpt) *PackageControllerClient {
...
func (pc *PackageControllerClient) Enable(ctx context.Context) error {
	...
	if err := pc.chartInstaller.InstallChart(ctx, chartName, ociURI, pc.chart.Tag(), pc.kubeConfig, constants.EksaPackagesName, valueFilePath, false, values); err != nil {
		return err
	}
...
func (pc *PackageControllerClient) ReconcileDelete(ctx context.Context, logger logr.Logger, client KubeDeleter, cluster *anywherev1.Cluster) error {
	...
	if err := pc.deleter.Delete(ctx, pc.kubeConfig, name, constants.EksaPackagesName); err != nil {

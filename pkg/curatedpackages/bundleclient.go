...
// BundleClient abstracts the source of package bundle information.
//
// Whether the source is a registry, a cluster, the BundleClient should
// abstract the details of retrieving a particular curated packages bundle.
...
type BundleClientOptions struct {
	...
	// restConfigurator is used to inject testing constructs.
	//
	// It shouldn't be required for normal use.
...
func NewBundleClient(source BundleSource, opts BundleClientOptions) (BundleClient, error) {
	...
	case Cluster:
		kubeClient, err := newKubeClient(opts.KubeConfig, opts.restConfigurator)
		if err != nil {
			return nil, err
		}

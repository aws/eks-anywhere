...
func TestOCIRegistryClient_PullBytes(t *testing.T) {
	sut := registry.NewOCIRegistry(registryContext)
	assert.NoError(t, sut.Init())
	bogusImage := registry.Artifact{
		Registry:   "localhost",
	...
	_, err := sut.PullBytes(context.Background(), bogusImage)
	assert.EqualError(t, err, "registry copy source: error creating repository !@#$: invalid reference: invalid repository")
}

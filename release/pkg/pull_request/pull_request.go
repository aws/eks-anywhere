...
func CreatePR(prc *types.PullRequestConfig) error {
	...
	err = updateLocalBranch(prc.GithubUser, prc.Branch)
	if err != nil {
		return err
	}

	err = commitAndCreatePR(prc)
	if err != nil {
		return err
	}
...
func updateLocalBranch(githubUser, branch string) error {
	originHTTPSURL := fmt.Sprintf("https://github.com/%s/eks-anywhere.git", githubUser)
	updateLocalBranchCommandSequence := fmt.Sprintf("git remote add %s %s; git remote add %s %s; git checkout -B %s; git stash; git fetch %s; git rebase -Xtheirs %s/%s; git stash pop", originHTTPSRemoteName, originHTTPSURL, upstreamHTTPSRemoteName, upstreamHTTPSURL, branch, upstreamHTTPSRemoteName, upstreamHTTPSRemoteName, branch)
...
func commitAndCreatePR(prc *types.PullRequestConfig) error {
	pwd, err := os.Getwd()
	if err != nil {
		return err
	}
	...
	addAndCommitFilesCommandSequence := fmt.Sprintf("%s git checkout -B %s; git commit -m '%s'", gitAddCommandSequence, prBranchName, commitMessage)
	...
	if prc.DryRun {
		fmt.Println("Dry-ru of PR creation successful")
		return nil
	}
	...
	ghPrCreateCommand := fmt.Sprintf("gh pr create --title '%s' --body '%s' --base '%s' --label 'do-not-merge/hold'", prTitle, prBody, prc.Branch)

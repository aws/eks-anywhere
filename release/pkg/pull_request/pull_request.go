...
func updateLocalBranch(githubUser, baseBranch, prBranch string) error {
	...
	updateLocalBranchCommandSequence := fmt.Sprintf("git remote add %s %s; git remote add %s %s; git checkout -B %s; git fetch %s; git reset --hard %s/%s", originHTTPSRemoteName, originHTTPSURL, upstreamHTTPSRemoteName, upstreamHTTPSURL, prBranch, upstreamHTTPSRemoteName, upstreamHTTPSRemoteName, baseBranch)

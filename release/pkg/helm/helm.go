...
func GetHelmDest(d *helmDriver, r *releasetypes.ReleaseConfig, ReleaseImageURI, assetName string) (string, error) {
	...
	fmt.Printf("Starting to modifying helm chart %s", helmChart[0])
	fmt.Printf("Pulling helm chart %s", ReleaseImageURI)
	...
	fmt.Printf("Untar helm chart %s into %s", chartPath, dest)
...
func ModifyAndPushChartYaml(i releasetypes.ImageArtifact, r *releasetypes.ReleaseConfig, d *helmDriver, helmDest string) error {
	...
	fmt.Printf("Checking inside helm chart for Chart.yaml %s", helmDest)
	...
	fmt.Printf("Overwriting helm chart.yaml version to new tag %s", chartYaml.Version)
	...
	fmt.Printf("Re-Packaging modified helm chart %s", helmDest)
	packaged, err := PackageHelmChart(helmDest)
	if err != nil {
		return fmt.Errorf("packaging the helm chart: %w", err)
	}
	fmt.Printf("Pushing modified helm chart %s to %s", packaged, r.ReleaseContainerRegistry)

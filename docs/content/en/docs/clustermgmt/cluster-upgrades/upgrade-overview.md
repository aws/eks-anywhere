...
As of EKS Anywhere version `v0.19.0`, if you are running EKS Anywhere on bare metal, you can use in-place upgrades for EKS Anywhere and Kubernetes versions, which upgrades the components on the same physical machines without requiring additional server capacity. In-place upgrades are not available for other providers.

...
- **Kubernetes version upgrades**: Skipping Kubernetes minor versions during upgrade is not supported (`v1.34.x` to `v1.36.x`). You must upgrade one Kubernetes minor version at a time (`v1.34.x` to `v1.35.x` to `v1.36.x`).

...
- Before you create a subscription, you must onboard to use AWS License Manager. See the AWS License Manager [documentation](https://docs.aws.amazon.com/license-manager/latest/userguide/getting-started.html) for instructions.
...
- An individual subscription can have up to 1000 licenses.
...
After the subscription is created, the next step is to apply the licenses to your EKS Anywhere clusters. Reference the [License EKS Anywhere Cluster page]({{< relref "./cluster-license">}}) for instructions.
...
Get subscription `id` based on subscription `name`
...
Get subscription `arn` based on subscription `name`

...
| server.retention | Prometheus-server data retention period | `"15d"` |
...
| serverFiles."prometheus.yml".scrape_configs | Specifies a set of targets and parameters for prometheus-server describing how to scrape them. Refer to Prometheus docs [scrape_config](https://prometheus.io/docs/prometheus/latest/configuration/configuration/#scrape_config) for more details | See footnote [^3] |
...
[^1]:
    Node-exporter is exposed as a clusterIP with `hostPort: 9100` and `servicePort: 9100` by default. Note the annotation `prometheus.io/scrape: "true"` is mandatory in order for node-exporter to be discovered by prometheus-server as a scrape target. See below specification for details.
      ```yaml
      apiVersion: packages.eks.amazonaws.com/v1alpha1
      kind: Package
      ...
...
[^2]:
    Node-exporter has the following toleration by default, which allows daemonSet to be scheduled on control plane node.
      ```yaml
      apiVersion: packages.eks.amazonaws.com/v1alpha1
      kind: Package
      ...

...
[^3]:
    Prometheus-server service is exposed as a `clusterIP` with `port: 9100` (controlled by `server.service.servicePort` below) and `targetPort: 9100` (not overridable) by default. See below specification for details.
      ```yaml
      apiVersion: packages.eks.amazonaws.com/v1alpha1
      kind: Package
      ...

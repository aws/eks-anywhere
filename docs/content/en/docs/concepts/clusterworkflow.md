...
Details about this config file are contained on the [Bare Metal Config]({{< relref "../reference/clusterspec/baremetal.md" >}}) page.
Create a hardware configuration file (`hardware.csv`) as described in [Prepare hardware inventory]({{< relref "../reference/baremetal/bare-preparation/#prepare-hardware-inventory" >}}).
...
See [Overview of Tinkerbell in EKS Anywhere]({{< relref "../reference/baremetal/netbooting-and-tinkerbell/#overview-of-tinkerbell-in-eks-anywhere" >}}) for examples of commands you can run to watch over this process.
...
The following diagram illustrates what happens when you start the cluster creation process, as described in the [vSphere Getting started]({{< relref "../getting-started/production-environment/vsphere-getstarted#create-an-initial-cluster" >}}) guide.
...
Details about this config file are contained on the [vSphere Config]({{< relref "../reference/clusterspec/vsphere.md" >}}) page.
...
With the target cluster up, and the CAPI service running on the target cluster, the bootstrap cluster is no longer needed and is deleted:
...
* A workload cluster (to run workloads) or
* A management cluster (to optionally create one or more workload clusters)


### Creating workload clusters (optional)

As described in [Create separate workload clusters]({{< relref "../getting-started/production-environment/vsphere-getstarted/#create-separate-workload-clusters" >}}), you can use the cluster you just created as a management cluster to create and manage one or more workload clusters on the same vSphere provider as follows:

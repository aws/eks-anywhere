...
Since EKS Anywhere clusters comes preinstalled with Helm, we will be using it for setting up Prometheus and Grafana.
{{% /alert %}}
...
2. Click on **Manage** submenu under **Dashboard** (in the sidecar) and select `Kubernetes / Compute Resources / Pod` to see the metrics of all the pods. Here is the view of the pods running the sample application:
    ...
    {{% alert title="Notes" color="warning" %}}
    Make sure to select the right namespace and pod name.
    {{% /alert %}}

...
### Step by step guide
  ...
  3. **Create a PersistentVolumeClaim**
    ...
    **Output**
    ...
    You can also check if the above PV is created under cluster > Monitor Tab > Cloud Native Storage in vSphere client. The PV is labelled with the metadata.labels provided in the PVC configuration.
  ...
  5. **Verify if the pod has the volume attached**
    ...
    **Output**
    ...
    Check if the associated volume is mounted as a Filesystem in the pod
    ...
    **Output**
    ...
    {{% alert title="Notes" color="primary" %}}
    Refer to [vsphere-csi-driver-examples](https://github.com/kubernetes-sigs/vsphere-csi-driver/tree/master/example) for information on creating other advanced storage constructs like Block Volumes, Zone based storage, Volumes with Read Write Many access, etc.
    {{% /alert %}}
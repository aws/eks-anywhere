...
To enable ADOT pods to authenticate with AWS services like AMP, you need to configure IAM Roles for Service Accounts (IRSA). This involves two separate steps: setting up the generic IRSA infrastructure for your cluster, and creating an ADOT-specific IAM role.

### Step 1: Setup Generic IRSA Infrastructure (One-time per cluster)

If you haven't already configured IRSA for your cluster, complete the generic IRSA infrastructure setup by following the [EKS Anywhere cluster spec for Pod IAM]({{< relref "../../getting-started/optional/irsa/" >}}) guide. This includes:

1. Creating an OIDC provider
1. Hosting the OIDC discovery document in an S3 bucket (recommend `us-west-2` region)
1. Deploying the pod-identity-webhook in your cluster

This is a **one-time setup** that enables any package or workload in your cluster to use IRSA for AWS authentication.

### Step 2: Create IAM Role for ADOT (ADOT-specific)

After the generic IRSA infrastructure is set up, create a dedicated IAM role specifically for ADOT pods:

1. Create an IAM role with a trust policy that allows the ADOT ServiceAccount to assume it. Replace `<ACCOUNT_ID>`, `<ISSUER_HOSTPATH>` with your values:
...
1. Save the IAM role ARN (e.g., `arn:aws:iam::123456789012:role/adot-amp-role`). You'll use this ARN in the ADOT package configuration below.

{{% alert title="Why a Dedicated IAM Role?" color="info" %}}
Each package or workload should have its own IAM role with only the permissions it needs (principle of least privilege). The ADOT package will create its own dedicated ServiceAccount and associate it with this IAM role, following Kubernetes and AWS security best practices.
{{% /alert %}}

### IRSA Set Up Test
To ensure IRSA infrastructure is set up properly in the cluster, you can create an `awscli` pod for testing:
...
EKS Anywhere supports two approaches for configuring ADOT: a simplified preset-based approach (recommended for most users) and an advanced approach with explicit RBAC rules (for custom configurations).
      ...
      spec:
        ...
        config: |
          ...
          serviceAccount:
            ...
            annotations:
              eks.amazonaws.com/role-arn: "<IAM-ROLE-ARN>"  # Replace with your IAM role ARN from Step 2 above
...
1. Install the ADOT package using the configuration file (same command for both approaches):
    ```bash
    eksctl anywhere create packages -f adot-package.yaml
    ```

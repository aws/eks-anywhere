...
The `$KUBEVERSION` should be set to the same as the spec.kubernetesVersion of your EKS Anywhere cluster specification. When using self-signed certificates for your registry, you should run with the --dst-insecure command line argument to indicate skipping TLS verification while copying curated packages. 

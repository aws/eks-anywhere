...
- For `main`, we use the next minor version to the latest tag available. For example, if the latest prod release is `v0.25.3`, the version used for dev releases will be `v0.26.0-dev+build.{number}`. This aligns with the fact that the code in `main` belongs to the next future prod release `v0.26.0`.
- For `release-*` branches, we use the next patch version to the latest available tag for that minor version. For example, for `release-0.25`, if the latest prod release for v0.25 is `v0.25.5`, dev releases will follow `v0.25.6-dev+build.{number}`.
